<?php
$speakers = $item->speakers()->_pages();
$day = $item->parent();

?>
<?php snippet('cards/default', ['item' => $item, 'url' => $item->url()], slots: true); ?>
<?php slot('before_title') ?>
<?php if (count($speakers)) : ?>
    <ul class="flex flex-wrap gap-1 mt-2">
        <?php foreach ($speakers as $speaker) : ?>
            <li class="tag text-sm yellow py-1 px-2 font-brown_bold"><?= $speaker->title() ?></li>
        <?php endforeach; ?>
    </ul>
<?php endif; ?>
<?php endslot() ?>
<?php slot('after_title') ?>
<div class=" text text-sm font-brown_bold text-darkGrey">
    <div><?= $item->_time_range() ?></div>
</div>
<?php endslot() ?>
<?php endsnippet(); ?>