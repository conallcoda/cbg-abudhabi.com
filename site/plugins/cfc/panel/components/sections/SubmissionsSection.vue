...
export default {
    ...
    methods: {
        async load(reload) {
            this.$options.extends.methods.load.call(this, reload);
        },
        async reload() {
            this.$options.extends.methods.reload.call(this);
        },
        onPaginate(pagination) {
            this.$options.extends.methods.onPaginate.call(this, pagination);
        },
        onSearchToggle() {
            this.$options.extends.methods.onSearchToggle.call(this);
        },
        search() {
            this.$options.extends.methods.search.call(this);
        },
        update() {
            this.$options.extends.methods.update.call(this);
        },
        onAdd() {
            if (this.canAdd) {
        ...
        async onChange(event) {
            ...
            if (type) {
                ...
                const position =
                    event[type].newIndex + 1 + this.pagination.offset;
